(function(window, N) {
    ...
    var CommonUtils = {
        /**
         * N.grid 나 N.list에서 체크(check)되거나 선택(select)된 행들을 삭제하기 위한 함수 - 삭제 전 동의 메시지 다이얼로그 표시 후 remove 함수 호출 등의 반복적인 로직을 한번에 처리해 줍니다.
         *
         * @this : 호출한 함수 인스턴스
        ...
        */
        ...
        /**
         * 추가, 수정, 삭제된 데이터를 저장하는 함수 - 저장 전 데이터 검증, 변경된 데이터 확인, 저장 메시지 다이얼로그 표시등 데이터 저장에 대한 반복적인 로직을 한번에 처리해 줍니다.
         *
         * @this : 호출한 함수 인스턴스
         * @opts.cont : N.cont object
        ...
        /**
        * N.grid 나 N.list의 행을 선택했을 때(onSelect 이벤트 핸들러 함수 이용) N.form 컴포넌트에 같은 데이터를 연동하기 위한 반복적인 로직을 한번에 처리해 줍니다.
        *
