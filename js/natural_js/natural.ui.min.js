		...
		$.extend(DatePicker.fn, {
			show : function() {
				...
				if(!N.string.isEmpty(opts.context.val())) {
					var iptVal = opts.context.val().replace(/[^0-9]/g, "");
					if(iptVal.length >= 4) {
						opts.contents.find("div.datepicker_years_panel__ div.datepicker_year_item__:contains('" + String(parseInt(iptVal.substring(0, 4))) + "')").click();
					}
					if(iptVal.length >= 6) {
						if(!opts.monthonly) {
							opts.contents.find("div.datepicker_months_panel__ div.datepicker_month_item__:contains('" + String(parseInt(iptVal.substring(4, 6))) + "')").click();
						} else {
							opts.contents.find("div.datepicker_months_panel__ div.datepicker_month_item__").removeClass("datepicker_month_selected__");
							opts.contents.find("div.datepicker_months_panel__ div.datepicker_month_item__:contains('" + String(parseInt(iptVal.substring(4, 6))) + "')").addClass("datepicker_month_selected__");
						}
					}
					if(iptVal.length >= 8) {
						opts.contents.find("div.datepicker_days_panel__ div.datepicker_day_item__").removeClass("datepicker_day_selected__");
						opts.contents.find("div.datepicker_days_panel__ div.datepicker_day_item__:contains('" + String(parseInt(iptVal.substring(6, 8))) + "')").addClass("datepicker_day_selected__");
					}
				}
		        ...
		        $(document).bind("keyup.datepicker", function(e) {
					...
					if($(e.target).closest(opts.context).length === 0 && $(e.target).closest(opts.contents).length === 0) {
						this_.hide();
					}
				});
			...
			hide : function() {
				...
				$(document).unbind("keyup.datepicker").unbind("click.datepicker");
			...
			wrapEle : function() {
				...
				monthItem.css({
					...
					if(opts.monthonly) {
						...
						selDate = N.date.strToDate(N.string.lpad(selDate.obj.formatDate(N.context.attr("data").formatter.date.Ym().replace(/[^Y|^m|^d]/g, "")), 6, "0"));
						...
						if(N.string.trimToEmpty(opts.context.val()).length >= 8) {
							// set context value acording to the month selecting
							opts.context.val(N.string.lpad(yearsPanel.find("div.datepicker_year_selected__").text(), 4, "0") +
									N.string.lpad(monthsPanel.find("div.datepicker_month_selected__").text(), 2, "0") +
									opts.context.val().substring(6, opts.context.val().length));

							daysPanel.find("div.datepicker_day_item__:contains(" + String(parseInt(opts.context.val().substring(6, 8))) + "):eq(0)").addClass("datepicker_day_selected__");
						} else {
				...
				if(!opts.monthonly) {
					...
					dayItem.css({
						...
						selDate = N.date.strToDate(N.string.lpad(selDate.obj.formatDate(N.context.attr("data").formatter.date.Ymd().replace(/[^Y|^m|^d]/g, "")), 8, "0"));
				...
				opts.context.bind("keyup.datepicker", function(e) {
					...
					if (e.keyCode >= 48 && e.keyCode <= 57) {
	        			...
	        			if(value.length === 4 && dateStrArr[0] < 100) {
        					opts.context.alert(N.message.get(opts.message, "yearNaN")).show();
        					opts.context.val("");
        					return false;
        				} else if(value.length === 6 && dateStrArr[1] < 1 || dateStrArr[1] > 12) {
        					opts.context.alert(N.message.get(opts.message, "monthNaN")).show();
        					opts.context.val(value.substring(0, 4));
        					return false;
        				} else if(!opts.monthonly && value.length === 8 && (dateStrArr[2] < 1 || dateStrArr[2] > parseInt(gEndDate))) {
        					opts.context.alert(N.message.get(opts.message, "dayNaN", [String(parseInt(gEndDate))])).show();
        					opts.context.val(value.substring(0, 6));
        					return false;
        				}
	        			...
	        			if(format === "Ymd" || format === "Ym") {
		        			...
		        			if(value.length === 4) {
		        				yearsPanel.find("div.datepicker_year_item__").removeClass("datepicker_year_selected__");
	        					DatePicker.yearPaging(yearsPanel.find("div.datepicker_year_item__"), value, -2, true);
	        					yearsPanel.find("div.datepicker_year_item__:contains('" + String(parseInt(value)) + "')").click();
		        			}
	        		...
	        		// when press the enter key
						...
						if(!opts.monthonly && value.length === 7) {
							value = "0" + value;
						} else if(opts.monthonly && value.length === 5) {
							value = "0" + value;
						}
