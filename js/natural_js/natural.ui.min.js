 ...
 */
(function(window, $) {
	N.version["Natural-UI"] = "v0.8.14.00";
	...
	(function(N) {
		...
		var Grid = N.grid = function(data, opts) {
			...
			// data filter
			if(this.options.filter || this.thead.find("> tr th[data-filter]").length > 0) {
				if(this.options.filter) {
					this.thead.find("> tr th").attr("data-filter", "string");
				}
				...
				var btnEle = $('<a href="#" class="btn_data_filter__" title="' + N.message.get(opts.message, "dFilter") + '"><span>' + N.message.get(opts.message, "dFilter") + '</span><a>')
					...
					.bind("click.grid.dataFilter", function(e) {
						...
						for(var k in filterKeys) {
							...
							var prevFilterItemEle = filterListBox.find(".data_filter_item_" + N.string.escapeSelectorChar(k) + "__");
							if(prevFilterItemEle.length > 0) {
								filterItemEle = prevFilterItemEle;
								filterItemEle.find(".data_filter_cnt__").text("(" + String(length) + ")");
							} else {
								filterItemEle = $('<li class="data_filter_item_' + N.string.escapeSelectorChar(k) + '__">'
						...
						bfrSelId = id;
					}).prependTo(theadCells.filter("[data-filter]"));
			},
        	checkAll : function() {
        		var opts = this.options;
    	        var thead = this.thead;
		...
		$.extend(Grid.prototype, {
			...
			bind : function(data) {
				...
				if(opts.filter || this.thead.find("> tr th[data-filter]").length > 0) {
