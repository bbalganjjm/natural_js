...
1. 이 작업의 계획은 docs/cookbook-working-plan.md 로 실행계획과 체크리스트를 생성하고 이미 생성되어 있다면 하나하나 체크하면서 작업을 누락없이 일관성있게 수행한다.
2. 작업이 끝날때마다 docs/cookbook-working-plan.md 파일에 끝낸 작업에대해서 체크하고 작업캐시를 비우고 다시 이터레이션을 시작한다.
1. playwright mcp server 를 사용해서 하나도 빠짐없이 세트별로(===로 구분) 모든 URL을 방문해서 자료를 수집해서 docs/COOKBOOK-{클래스나 컴포넌트, 오브젝트 명}.md 파일에 기록한다.
   - 가능하면 playwright headless 를 사용하여 정보를 수집한다.
   - 내가 원작자이므로 저작권은 신경쓰지 않아도 된다.
   - 채팅창에 표시하지 말로 반드시 docs/COOKBOOK-*.md 파일에 기록한다.
