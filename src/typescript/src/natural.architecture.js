...
N.prototype.ajax = function(opts) {
    return N.ajax(opts);
};
N.prototype.comm = function(url) {
    return new N.comm(this, url);
};
N.prototype.request = function() {
    return this.get(0).request;
};
N.prototype.cont = function(contObj) {
    return new N.cont(this, contObj);
};

N.comm.constructor = function(obj, url) {
    if (obj === undefined) {
    ...
    if(N.config.filterConfig === undefined) {
        N.config.filterConfig = N.comm.initFilterConfig();
    }

    var isFilterStopped = false;
    // request filter
    $(N.config.filterConfig.beforeInitFilters).each(function(i) {
        var jo;
    ...
    obj.request = new N.comm.request(obj, N.isString(url) ? {
        "url" : url
    } : url);

    obj.errorHandlers = [];

    $($.map(N.comm, function(v, i){
        if(typeof N.comm[i] === "function") {
            if(i !== "request") {
                return i;
            }
        }
...
N.comm.xhr = null;
N.comm.initFilterConfig = function() {
...
N.comm.resetFilterConfig = function() {
    N.config.filterConfig = N.comm.initFilterConfig();
    return this;
};
N.comm.submit = function(callback) {
    var obj = this;
    if (N.isElement(obj)) {
    ...
    var isFilterStopped = false;
    // request filter
    $(N.config.filterConfig.afterInitFilters).each(function(i) {
    ...
    $.extend(obj.request.options, {
        beforeSend : function(xhr, settings) {
            var isFilterStopped = false;
            // request filter
            $(N.config.filterConfig.beforeSendFilters).each(function(i, filter) {
        ...
        success : function(data, textStatus, xhr) {
            var isFilterStopped = false;
            // request filter
            $(N.config.filterConfig.successFilters).each(function(i, filter) {
            ...
            if (!N.isElement(obj)) {
                if (obj.request.options.urlSync && obj.request.options.referrer.replace(/!/g, "") != window.location.href.replace(/!/g, "")) {
                    xhr.abort();
                    N.warn("[N.comm.submit.success(urlSync option)]The response was stopped because it was different from the URL at the time of the request and the URL at the time of the response.");
                    return false;
                }
            } else {
                ...
                if(obj.request.options.replace){
                    if(obj.nextAll(".view_context__:first").length > 0){
                        ...
                        if(cont !== undefined){
                            ...
                            N.cont.trInit.call(this, cont, obj.request);
                        }
                    }
                    obj.remove();
        ...
        error : function(xhr, textStatus, e) {
            var isFilterStopped = false;
            // request filter
            $(N.config.filterConfig.errorFilters).each(function(i, filter) {
        ...
        complete : function(xhr, textStatus) {
            var isFilterStopped = false;
            // request filter
            $(N.config.filterConfig.completeFilters).each(function(i, filter) {
...
N.comm.error = function(callback) {
    this.errorHandlers.push(callback);
    return this;
};

N.comm.request.constructor = function(obj, opts) {
    this.options = {
        url: null,
 ...
 */
N.comm.request.prototype.attr = function(name, obj_) {
    if (name === undefined) {
        return this.attrObj;
    }
    if (obj_ === undefined) {
        ...
        // this.obj is defined at N.comm.request constructor;
        return this.obj;
    }
};
/**
 ...
 */
N.comm.request.prototype.removeAttr = function(name) {
 ...
 */
N.comm.request.prototype.param = function(name) {
    if (N.isEmpty(name)) {
...
N.comm.request.prototype.get = function(key) {
    if(key !== undefined) {
 ...
 * Reload block page
 */
N.comm.request.prototype.reload = function(callback) {
...
// Controller
N.cont.constructor = function(obj, contObj) {
 ...
 * "init" method trigger
 */
N.cont.trInit = function(cont, request) {
    // set request attribute
    cont.request = request;

    // AOP processing
    N.cont.aop.wrap.call(this, cont);
    ...
    wrap : function(cont) {
        if (N.context.attr("architecture").cont &&
            ...
            $(o.advisors).each(function (idx, advisor) {
                ...
                pointcut = o.pointcuts ? (o.pointcuts[advisor.pointcut.type] || N.cont.aop.pointcuts[advisor.pointcut.type]) : N.cont.aop.pointcuts[advisor.pointcut.type];
...
N.context.attrObj = {};
N.context.attr = N.comm.request.prototype.attr;

N.config.filterConfig = undefined;