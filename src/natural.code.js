 ...
 */
(function(window, $) {
	N.version["Natural-CODE"] = "0.1.1";

	(function(N) {

		var CODE = N.code = {
			inspection : function(sourceCode, excludes) {
			    var reportObj = []
			    
			    /**
			     * jQuery Selector 에 view 차가 안한 코드 검출
			     */
			    var regex = /[N$]\((.*?)\)(.*)/gm;
			    var match;
			    while (match=regex.exec(sourceCode)) {
			        ...
			        if((new RegExp(/["']/, "g")).test(selector)) {
    			        if(!isExclude) {
    	                    if((new RegExp(/[\(\)]|,view|,cont.view|",|',|^"<|^'<|>"$|>'$/, "g")).test(selector)) {
    	                        isExclude = true;
    	                    }
    	                    
    	                    if(!(new RegExp(/,view|,cont.view/, "g")).test(selector)
                                    && (new RegExp(/"\+|'\+|\+"|\+'/, "g")).test(selector)) {
                                isExclude = false;
    	                    }
    			        }
    			        ...
    			        if(!isExclude) {
    			            var method = match[2];
    			            if((new RegExp(/^.cont\(|^.comm\(|^.select\(|^.form\(|^.list\(|^.grid\(|^.pagination\(|^.tree\(/, "g")).test(method)) {
    	                        isExclude = true;
    	                    }			            
    			        }
			...
			appendSourceURL : function(sourceCode, sourceURL) {
