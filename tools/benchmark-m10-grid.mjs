...
// node tools/benchmark-m10-grid.mjs --initial-page=1 > initial-page.json
...
function argumentsFrom(argv) {
  ...
  for (const arg of argv) {
    const match = /^--(source|compare-source|rows|fields|page-size|initial-page|runs|warmup)=(.+)$/.exec(arg);
  ...
  const initialPage = values.has("initial-page") ? positive("initial-page", 1) : null;
  ...
  return { source, compareSource, rows, fields, pageSize, initialPage, runs, warmup };
}
...
async function measureSource(browser, baseURL, source, options) {
  ...
  for (let index = 0; index < options.warmup + options.runs; index++) {
    ...
    try {
      ...
      await page.evaluate(async ({ modules, rows: count, fields: fieldCount, pageSize, initialPage }) => {
        ...
        window.m10GridBenchmark = {
          bind() {
            ...
            this.grid = bindGrid(table, {
              rows: store,
              ...(initialPage === null ? {} : { initialPage: { page: initialPage, size: pageSize } })
            });
            const bindMs = performance.now() - start;
            const renderedRows = table.tBodies[0].rows.length;
            const expectedRows = initialPage === null ? count : pageSize;
            if (renderedRows !== expectedRows) throw new Error(`Initial Grid render: ${renderedRows} rows, expected ${expectedRows}.`);
            const actualPage = this.grid.page();
            if (initialPage === null) {
              if (actualPage !== null) throw new Error("Initial Grid page state unexpectedly enabled paging.");
            } else if (actualPage?.page !== Math.min(initialPage, pageCount) || actualPage.size !== pageSize ||
                       actualPage.total !== count || actualPage.pages !== pageCount) {
              throw new Error("Initial Grid page state did not match the requested page.");
            }
      ...
      }, { modules, rows: options.rows, fields: options.fields, pageSize: options.pageSize, initialPage: options.initialPage });
  ...
  try {
    ...
    return {
      ...
      method: {
        ...
        runs: options.runs,
        newPagePerRun: true,
        initial: options.initialPage === null
          ? "Create Rows before timing; bindGrid renders all rows before paging."
          : `Create Rows before timing; bindGrid starts at page ${options.initialPage} with ${options.pageSize} rows per page.`,
        navigation: options.initialPage === null
          ? "Set page 1, traverse page 2 through last page, then return to page 1; measure synchronous setPage only."
          : "Explicitly set page 1 after bind (a repeat request when initialPage=1), then traverse page 2 through last page and return to page 1; measure synchronous setPage only.",
      ...
      ...(options.initialPage === null ? {} : { initialPage: options.initialPage }),
