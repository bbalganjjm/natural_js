...
The current runtime provides `mountPage`, `createRows`, `createCommunicator`, `bindForm`, `bindGrid`, and `FrameworkError`. Form and Grid are M4 pilots: they support a complete search, selection, edit, and save example, including nested row-local Select options. The retained 1.x built-in formatter and validator rules are planned for M5; the remaining first-release UI components follow in M6 and M7. See [the active plan](https://github.com/bbalganjjm/natural_js/blob/2.0.0-alpha.0/docs/implementation/current.md) and [the two-layout employee example](https://github.com/bbalganjjm/natural_js/blob/2.0.0-alpha.0/docs/v2/employee-example.md) for the exact implemented boundary.
