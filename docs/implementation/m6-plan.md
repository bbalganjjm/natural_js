...
generated: { by: codex/gpt-6-sol, at: 2026-09-24T11:26:11Z }
---
...
| List | Repeat one authored `<li>` template over `Rows`, show fields and row-local options, support row-keyed selection and basic editing/validation. | No timer-driven row creation, scroll paging engine, arbitrary HTML insertion, or index-based identity. |
...
| 5. List | Bind one authored `<li>` template to `Rows` with cached field descriptors and row-local references. Add a real row-selection control, basic editable fields through the same rule contract, and an authored empty-state region. Preserve draft and selection identity through sort/filter/page operations. | Add/update/delete/revert, hidden-row validation, nested option arrays, keyboard selection, and repeated bind/dispose pass without per-row document queries or Form instance duplication. |
...
| 8. Cost and documentation gate | Compare the fixed M4/M5 fixture after each large Grid/List change; inspect code/API growth and an independent docs-first agent task. Update affected English OKF concepts, indexes, log, fingerprints, examples, migration notes, and active checkpoint with code. | Build/typecheck, focused logic and browser checks, JS/TS installed consumers, changed OKF check with zero errors, full OKF findings recorded, package audit, and measurements support a user review before M7. |
