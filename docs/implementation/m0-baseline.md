...
verified:
  - { by: codex/gpt-6-sol, at: 2026-09-24T06:51:46Z }
---
...
Preserve CVC and behavior attached to authored HTML while migrating only functions required by named 2.0 framework behavior. Make application-specific formatting, validation, data transformations, and convenience functions the application's responsibility.
...
- Repeated row HTML uses duplicate DOM ids as data keys; row indexes become unstable under sorting and filtering. Use a separate field binding marker and stable row identity. [^ui]
...
| N.formatter and N.validator integration | Show and edit values; report invalid fields | User conversion/validation callback invocation, HTML validity, result display | Public callback contract; private orchestration, M1/M5 |
| N.ui row iteration | List and grid from authored templates | Clone or instantiate author-supplied repeat template and bind row values | Private per component, M4/M6 |
...
| N.string, N.array, N.json, N.browser, generic N.date, and Date.prototype extensions | Exclude | JavaScript/browser standards or application code |
| N.message and N.locale as generic lookup libraries | Exclude | Application translation and business text; localizable text only within a component that needs it |
...
| N.mask and formatter/validator engines and rule catalogs | Exclude | Application-owned formatting, masking, and validation functions |
...
| N.tree and N.datepicker | Defer | M11, with date calculations private to date picker |
| N.notify and N.docs | Defer | M12 shell integrations |

An existing call inside 1.x does not make a utility necessary in 2.0. Rewrite the call site around the owning feature or standard API before deciding whether any helper remains.
...
- Layout A places the search/detail form next to the table; layout B places it below the table. Use the same controller, data, actions, and fixtures in both. Authored classes, non-repeated structure, and CSS stay in place.
- Task 1: add a search/list/detail page from authored HTML. Both layouts pass the same behavior and keyboard scenarios.
- Task 2: add an email field, user validation callback, and save payload update. Invalid data sends zero requests; valid changes send only changed rows.
...
- Keep application rules outside the framework; do not create a separate legacy utility package.
- Keep 1.x source and docs at the fixed baseline commit. New 2.0 usage concepts describe only implemented behavior.
- The M4 representative screen is the structural gate before broad UI implementation.
