...
- The user chose a full-feature interactive Grid demo as the M10 reference screen. M10.1 kept grouped/sticky headings in authored HTML/CSS; M10.2 released offscreen records; M10.3 added the approved opt-in Grid `initialPage?: PageRequest` without changing the default. The 5,000-row/50-visible-row first bind measured 3.0 ms and 1,347 DOM nodes versus 96.2 ms and 105,297 nodes for the unchanged default on the same source. The fixed 1,000-row budgets pass. M10.4's docs-first screen passed in Chromium and Firefox after one Vite module-URL correction, and the rebuilt unpublished tarball passed installed JS/TS and Chromium, Firefox, real Chrome, and real Edge consumers. [M10 evidence and limits](m10-plan.md) records raw measurements, task cost, and exclusions. Column state, bulk edit, and multi-selection remain unselected; M11-M12 are later stages.
...
- Safari is outside the initial 2.0 support scope by user decision. Historical WebKit results are engine evidence only; current M10.3 reruns use Chromium, Firefox, and installed Chrome/Edge. The default Firefox Playwright cache fails to spawn on this host; the repository-local cache passes. Current WebKit execution was rejected by automatic approval review as outside the Safari-excluded scope.
