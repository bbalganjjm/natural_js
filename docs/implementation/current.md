...
generated: { by: codex/gpt-6-sol, at: 2026-09-24T20:26:19Z }
---
...
- [M9](m9-plan.md) is approved and in progress. The working manifest is an unpublished `2.0.0-beta.0` candidate. The user approved removing only the unused public `Rule` type; Form formatter/validator runtime stays. The clean source commit `c79b9eae87d089d36813f2a4e9c6ed5981891bb8` produced the [M9 beta artifact](m9-beta-report.md), with exact-tarball consumer and five-browser checks complete. It is now superseded by a narrow Popup focus and reference-layout reflow fix; repeat artifact-dependent checks after a new clean source commit. A fixed docs-first CVC task passed its first complete acceptance with zero code retries; its metered result is in the M9 report.
...
Finish the accessibility fix, commit a clean source checkpoint, repack the beta, and repeat artifact-dependent tests. Then update the M9 report, commit, and push the branch checkpoint. Bring the exact artifact, remaining Safari/manual-accessibility and npm-auth gates, and the trust-field warnings back for release-scope and publication review. Do not treat Playwright WebKit as a real Safari test.
...
| 2026-09-25 | M9 replay and raw logs | A fresh agent repaired the fixed delayed CVC example on the first complete acceptance: Chromium 2/2, zero code retries, 17 files and 85,572 metered output bytes in 6m53s. Source suite logs now preserve 103/103 per engine; exact-tarball logs preserve JS/TS and five-browser results. The [M9 beta report](m9-beta-report.md) lists limits and links raw evidence. |
...
- No Mac is available, so real Safari is unverified. Chrome and Edge packed-artifact smokes passed on Windows; real Safari remains unverified.
