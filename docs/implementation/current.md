...
- [M7 page UI](m7-plan.md) is a review draft. Dialog, Popup, and fixed Tabs have no 2.0 implementation or public exports yet; M7 needs separate user approval.

# Next action

Review the M7 plan against the M1 result and CVC lifecycle contract. Freeze compact HTML/TypeScript usage examples and resolve its host, revisit, and async-switch decisions with the user before implementation. Do not treat M6 approval as M7 approval.
...
- The system-installed Firefox does not expose the Playwright protocol. The fresh Playwright Firefox build passed M6; reproduce with `PLAYWRIGHT_BROWSERS_PATH=node_modules/.cache/playwright-m6` for Firefox only. Chromium/WebKit use the default cache.
