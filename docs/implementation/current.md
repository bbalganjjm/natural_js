...
- The user chose a full-feature interactive Grid demo as the M10 reference screen. The earlier [two authored layouts](../v2/advanced-grid-example.md) remain M10.1 grouped/sticky regression fixtures, with four focused cases passing in each of Chromium, Firefox, and WebKit. The [M10 plan](m10-plan.md) now treats every supported Grid option and behavior as demo coverage while keeping unimplemented advanced operations explicitly separate. M10.2 now releases offscreen Grid records; the [interactive Grid demo](../v2/grid-demo.md) exposes every implemented option, method, and declarative marker. Chromium, Firefox, and WebKit each passed 5/5 demo cases; focused Grid lifetime/legacy cases passed 12/12 per engine. The 5,000-row same-process comparison reduced post-traversal DOM nodes from 105,238 to 1,288 while page traversal rose about 27–30%; the fixed 1,000-row budgets still pass. [M10 evidence and limits](m10-plan.md) remain open for agent/package review. M11-M12 remain later stages.
...
Review the completed Grid demo and M10.2 evidence, then review the [M10.3 `initialPage` proposal](m10-plan.md) for the measured initial DOM peak. Implement it only after its new public contract is approved; column state, bulk editing, and multi-selection still need a real caller and separate scope. Do not treat historical 1.x options as an automatic checklist. The 5,000-row workload remains a performance fixture rather than a demo default, and initial full rendering is still an open peak-cost limit. Keep the exact M9 beta artifact as unpublished evidence; record manual accessibility, Node/tooling, and trust-field limits without claiming they passed. Do not publish to npm or create a release tag until the user requests it again.
