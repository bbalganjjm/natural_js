...
- M0-M4 are complete on branch `2.0.0-alpha.0`; the M4 screen, measurements, and final checks are recorded. M5 is a draft requiring separate approval. See [current.md](current.md).
...
Close the final M4 checks and fast-forward push the result to `origin/2.0.0-alpha.0`. Review [the M5 data and rule plan](m5-plan.md) and obtain separate approval before implementing M5.
