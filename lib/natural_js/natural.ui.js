		...
		$.extend(Alert, {
			...
			resetOffSetEle : function(opts) {
				if(opts.context.outerWidth() > 0) {
					var contextOffset = opts.context.position();
					opts.msgContext.css({
						"top" : opts.isWindow ? 0 : contextOffset.top + "px",
						"left" : opts.isWindow ? 0 : contextOffset.left + "px",
					...
					opts.msgContents.css({
						"top" : ((opts.msgContext.height() / 2 + contextOffset.top) - opts.msgContents.height() / 2) + "px",
						"left" : ((opts.msgContext.width() / 2 + contextOffset.left) - parseInt(opts.msgContents.width() / 2)) + "px"
					}).show();
				} else {
					// for non-active tab
					opts.msgContext.hide();
